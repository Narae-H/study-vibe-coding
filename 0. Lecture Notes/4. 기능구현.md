...
## 로그인 및 권한 분기 (Authentication & Guard)

### 개요

- 로그인 관련 기능은 **인증(Authentication)** 과 권한 **분기(Guard)** 로 구성됨.
- 일부 API 및 기능은 로그인한 사용자만 접근할 수 있으며, 비로그인 사용자는 제한됨.
- 따라서 기능별로 **로그인 필요 여부**를 명확히 구분해야 함.
- 로그인(Authentication)과 권한분기 (Guard) provider의 위치
  ```ts
    <AuthProvider>              // 로그인
      <ReactQueryProvider>
        <NextThemesProvider>
...
| 구분              | 설명                                     | 예시                       |
| --------------- | -------------------------------------- | ------------------------ |
| **로그인 필요 API**  | 사용자 인증이 필요한 API. Access Token 검증이 필요함. | 마이페이지 조회, 일기 작성, 댓글 등록 등 |
| **로그인 불필요 API** | 누구나 접근 가능한 API. 인증 토큰 없이 호출 가능.        | 게시글 목록 조회, 인기 일기 조회 등    |

- 이 구분에 따라 페이지 및 액션 단위에서 Guard 로직이 설정됨.

### 2. Guard(가드)란?

- Guard는 **특정 조건(로그인, 권한 등)** 에 따라 접근을 제한하거나 허용하는 로직.
- 크게 두 종류로 구분됨:
  - **Page Guard (페이지 보호)**: 비로그인 사용자가 특정 페이지에 접근하지 못하도록 제한
  - **Action Guard (행동 보호)**: 비로그인 사용자가 특정 기능(버튼 클릭 등)을 실행하지 못하도록 제한

#### 2-1. Page Guard (페이지 가드) 
- 비로그인 상태에서 로그인 필요한 페이지로 진입할 경우, 접근을 차단하고 로그인 유도.
- 일반적으로 Route Guard 또는 PrivateRoute 형태로 구현.
- Guard 구현 시에 URL 상수를 넣어서 처리
...
**처리 시나리오:**
1. 사용자가 로그인 상태인지 확인 (isAuthenticated 등)
2. 로그인되지 않았다면 “로그인이 필요한 서비스입니다.” 또는 “로그인 해주세요” Modal 표시
3. 로그인 완료 후 원래 시도했던 페이지로 리다이렉트

#### 2-2. Action Guard (액션 가드)
- 페이지 접근은 가능하지만, 로그인하지 않으면 특정 행동이 제한됨.
...
**처리 시나리오:**
1. 버튼 클릭 시 로그인 여부 검사
2. 비로그인 상태라면 “로그인이 필요한 서비스입니다.” Modal 표시
3. 로그인 후 해당 액션 재시도 가능

### 3. Guard 설계 시 주의사항
- Guard 로직은 **UI 레벨(컴포넌트)** 과 비즈니스 **로직(API 호출 전)** 모두에 적용해야 함.
- 로그인/권한 관련 기능은 복잡도가 높기 때문에, **유저 시나리오**와 **테스트 시나리오**를 충분히 설계해야 함.
- 예시 시나리오:
- [비로그인 → 마이페이지 접근] → Modal 표시 → 로그인 → 마이페이지 이동
- [로그인 → 상품 등록 → 만료된 토큰] → 자동 로그아웃 처리 → 로그인 화면 이동

### 4. 테스트 포인트
- Guard가 올바르게 작동하는지 확인할 주요 포인트:
  - 비로그인 사용자의 접근 제한이 정상적으로 동작하는가
  - 로그인 후 Guard가 해제되어 정상 접근 가능한가
  - 로그인 후 이전 페이지로 리다이렉트되는가
  - 토큰 만료 시 Guard 로직이 재적용되는가
