...
## 링크 연결
여기서 만들어야하는 링크는 모두 다 레이아웃에 있음 => `src/commons/layout/prompts` 안에서 프롬프트 실행
- 레이아웃 > 헤더 > 배너 이미지 링크 
- 레이아웃 > 네비게이션 > 사진보관함 링크
...
### 프롬프트 텍스트 파일
- [Link 기능 텍스트 파일](4.%20기능구현/prompt.301.func.link.routing.txt)
...
이 과정에서는 채팅창을 보지 않고 진행해야, 이후 병렬적으로 기능 개발이 가능.   
단, 아래의 리체크 룰 절차를 모두 수행했음에도 불구하고 비정상적인 결과가 발생할 수 있음.   
...
3. **Optional Recheck Rules** 실행 (필요 시 선택적으로 진행)
