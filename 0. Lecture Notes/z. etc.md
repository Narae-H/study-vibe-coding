...
# 2025/11/19 라이브 내용 정리: `프롬프트 라이브러리화`

## 1. 과거 방식 vs 현대 방식

| 과거 개발 방식 | AI 활용 방식 |
|-------------| ---------- |
| 기능 1개씩 손코딩 | 기능 여러 개 동시 코딩 (AI 코딩) | 
| **모듈**이 곧 자산 | **프롬프트**가 곧 자산 |

즉, 예전에는 재사용 가능한 **코드 모듈**이 핵심 자산이었다면  
AI 시대에는 잘 정리된 **프롬프트 라이브러리**가 생산성을 좌우하는 핵심 자산이 됨.

## 2. 프롬프트 라이브러리 구축 방법

1) 기존에 잘 동작했던 프롬프트를 가져와서, 현재 화면/기능에 맞게 살짝 수정  
2) 그렇게 만든 프롬프트로 실제 UI/기능을 구현  
3) 다른 페이지·다른 프로젝트에서도 재사용할 수 있도록 **표준화(템플릿화)** 해서 쌓아두기  

표준화된 프롬프트는 **템플릿**이므로,
- 공통적으로 꼭 필요한 구조/설명만 남겨두고  
- 실제 값(페이지명, 필드명, 도메인 용어 등)은 사용할 때 채워 넣는 방식으로 운용.

프롬프트를 모아 둘 전용 폴더를 하나 만들고, 계속 업데이트하면서 키워나감.
...
- **프롬프트 파일 이름 예시**
  ...
  - `prompt.201.ui.components.txt`
  - `prompt.201.ui.icon.txt`
  - `prompt.201.ui.modal.txt`
  - `prompt.301.func.binding.list.txt`  // 무한 스크롤 리스트, react-window 가상 스크롤 등
  - `prompt.301.func.binding.one.txt`   // 상세보기
  - `prompt.301.func.filter.txt`        // 필터 공통
  - `prompt.301.func.filter.localstorage.txt` // Local Storage 기반 필터
  - `prompt.301.func.filter.api.txt`          // API 기반 필터
  ```

  > 네이밍 규칙을 `prompt.번호.카테고리.기능` 형태로 맞춰두면,  
  > 나중에 “필터 관련 프롬프트”, “UI 컴포넌트 프롬프트” 등을 빠르게 찾을 수 있음.

## 3. 프롬프트 라이브러리 활용 노하우 (심화)

- **목표**: 프로젝트를 병렬로 빠르게 진행하기 위해, 프롬프트 작업도 미리 분할해 두는 것  
- 전제: 디자인·요구사항은 어느 정도 확정되었다고 가정하고, 그 이후 단계부터 바이브 코딩 진행

프롬프트 라이브러리는 **엑셀 시트**로 관리하면 편함.  
row(행)는 서로 병렬로 진행 가능한 작업 단위,  
column(열)은 한 작업 안에서 순서가 있는 단계(병렬 불가)를 의미.

아래와 같은 테이블을 폴더/카테고리 별로 묶어서 정리:

| 구현 | 작성 | 담당자 | [공통상수] | 비고 |
| --- | --- | ----- | -------- | ---- |
| O/X | O/X | 홍길동  | src/commons/constants/prompts/prompt.101.color.txt |  |

- **[공통상수]**: 컬럼 이름은 변경 가능하며, 파일 이름(경로 포함)만 먼저 적어 두는 칸  
  - 처음부터 프롬프트 내용을 다 써 넣는 것이 아니라,  
  - “어떤 프롬프트를 만들지” **자리와 이름만 먼저 잡아두고** 점진적으로 내용을 채워 넣는 방식.
